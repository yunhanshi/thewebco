...
trait HasPermissions
{
     ...
     */
    public function hasPermissionTo($permission, $guardName = null, $app = null): bool
    {
        ...
        if (is_string($permission)) {
            $permission = $permissionClass->findByNameAndApp(
                $permission,
     ...
     * @return bool
     */
    public function checkPermissionTo($permission, $guardName = null, $app = null): bool
    {
        try {
            return $this->hasPermissionTo($permission, $guardName, $app);
     ...
     * @throws \Exception
     */
    public function hasAnyPermission($permissions, $app = null): bool
    {
        $permissions = collect($permissions)->flatten();

        foreach ($permissions as $permission) {
            if ($this->checkPermissionTo($permission, null, $app)) {
                return true;
            }
        }
     ...
     * @throws \Exception
     */
    public function hasAllPermissions($permissions, $app = null): bool
    {
        $permissions = collect($permissions)->flatten();

        foreach ($permissions as $permission) {
            if (! $this->hasPermissionTo($permission, $app)) {
                return false;
            }
        }
     ...
     * @throws PermissionDoesNotExist
     */
    public function hasDirectPermission($permission, $app = null): bool
    {
        $permissionClass = $this->getPermissionClass();

        if (is_string($permission)) {
            $permission = $permissionClass->findByNameAndApp($permission, $this->getDefaultGuardName(), $app);
        }
